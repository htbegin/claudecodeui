...
import { Search, Filter, ArrowUpDown, ArrowUp, ArrowDown, List, Grid, ChevronDown, Columns, Plus, Settings, FileText, HelpCircle, X } from 'lucide-react';
...
const TaskList = ({ 
  ...
  const [isInitializingTaskMaster, setIsInitializingTaskMaster] = useState(false);
  const [taskMasterInitError, setTaskMasterInitError] = useState('');
  ...
  const handleInitializeTaskMaster = async () => {
    if (!currentProject?.name || isInitializingTaskMaster) return;

    setIsInitializingTaskMaster(true);
    setTaskMasterInitError('');

    try {
      const response = await api.taskmaster.init(currentProject.name);
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error?.message || 'Failed to initialize TaskMaster');
      }
      ...
      setTaskMasterInitError(error.message || 'Failed to initialize TaskMaster');
    } finally {
      setIsInitializingTaskMaster(false);
    }
  };
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="space-y-3">
              <button
                ...
                disabled={isInitializingTaskMaster}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg font-medium transition-colors flex items-center gap-2 mx-auto"
              >
                <Settings className="w-4 h-4" />
                {isInitializingTaskMaster ? 'Initializing...' : 'Initialize TaskMaster AI'}
              </button>
              {taskMasterInitError && (
                <p className="text-sm text-red-600 dark:text-red-400">{taskMasterInitError}</p>
              )}
            </div>
          </div>
        ) : (
